...
const generateRandomPoints = (centerLat, centerLng, radius, count, intensityModifier) => {
  ...
  for (let i = 0; i < count; i++) {
    ...
    const lng =
      centerLng +
      (distance * Math.sin(angle)) / (111.32 * Math.cos(centerLat * Math.PI / 180));
...
export default function NationalHealthPulse() {
  ...
  // update layer when disease or settings change
  useEffect(() => {
    if (heatLayerRef.current) {
      const d = mockData[activeDisease];
      heatLayerRef.current.setLatLngs(d.points);
